...
def get_person_detection_boxes(model, img, threshold=0.5):
    ...
    transform = transforms.Compose(
        [transforms.ToTensor()])  # Defing PyTorch Transform
...
def get_pose_estimation_prediction(pose_model, image, centers, scales, transform):
    ...
    for center, scale in zip(centers, scales):
        trans = get_affine_transform(
            center, scale, rotation, cfg.MODEL.IMAGE_SIZE)
...
def parse_args():
    ...
    parser.add_argument('--pic_fmt', type=str, required=True)
...
def main():
    ...
    pose_dir = prepare_output_dirs(args.opath)
    csv_output_rows = []
    ...
    # Loading an video
    opath = os.path.expanduser(args.opath)
    if not os.path.exists(opath):
        os.makedirs(opath)
    flist = [f for f in os.listdir(args.ipath)
             if f.endswith("." + args.pic_fmt)]
    img_list = []
    for f in flist:
        total_now = time.time()
        iname = os.path.join(args.ipath, f)
        image_bgr = cv2.imread(iname)
        ...
        if args.writeBoxFrames:
            for box in pred_boxes:
                cv2.rectangle(image_debug, (int(box[0][0]), int(box[0][1])),
                              (int(box[1][0]), int(box[1][1])), color=(0, 255, 0),
        ...
        img_file = os.path.join(pose_dir, f)
    ...
    csv_output_filename = os.path.join(opath, 'pose-data.csv')
